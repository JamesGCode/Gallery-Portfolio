...
function render() {
  ...
  items
    ...
    .forEach((item) => {
      const left = centerX + item.x - config.slideWidth / 2;

      const top = slider.clientHeight - config.slideHeight;

      item.slide.style.zIndex = Math.round(item.scale * 1000);

      item.slide.style.transform = `translate3d(${left}px, ${top}px, 0) scale(${item.scale})`;
    });
